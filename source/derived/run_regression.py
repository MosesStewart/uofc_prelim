...
def do_regress(dvars, method, spec1, spec2, outdir):
    ...
    for spec in range(1, 3):
        for wk in range(0, 2):
            ...
            output[f'Week {wk} ({spec}) Coeff'] = np.concatenate([formatted_est, fill])
            formatted_se = np.array( [f'({se:.3f})' for se in reg.bse.values[1:]])
            fill = ['.' for i in range(num_rows - formatted_se.shape[0])]
            output[f'Week {wk} ({spec})'] = np.concatenate([formatted_se, fill])
        output.to_latex(f'{outdir}/{method}_{spec}.txt', index = False)
